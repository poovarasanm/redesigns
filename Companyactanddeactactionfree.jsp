   ...
   if(companyact.equals("Activate"))
      {
	   ...
	   for(int j=0;j<id.length;j++)
          {
          ps=conn.prepareStatement(" update company_registration set CR_act_deact=1 where CR_code_id='"+id[j]+"'");
		  ps.execute();
	  ...
	  if(companyact.equals("Deactivate"))
      {
	   ...
	   for(int j=0;j<id.length;j++)
          {
          ps=conn.prepareStatement(" update company_registration set CR_act_deact=0 where CR_code_id='"+id[j]+"'");
		  ps.execute();
		  ps.close();
	  ...
	  if(companyact.equals("Delete"))
      {
	   ...
	   for(int j=0;j<id.length;j++)
          {
          ps=conn.prepareStatement(" delete from company_registration  where CR_code_id='"+id[j]+"'"            );
		  ps.execute();
		  ps.close();
...
<% if(companyact.equals("Userlist"))
	...
	<form>
	 ...
	 <tr>
	   <td colspan="7"><a href="/admin/Companyactanddeactfree.jsp">Click here to go back to Free Company act/deact page</a></td>
	 </tr>
        <tr>
	   <td width="5%">SL.No</td>
	   ...
	   <%
	     id=request.getParameterValues("compid");
	      for(int j=0;j<id.length;j++)
          {
          ps=conn.prepareStatement(" select CR_name ,CR_industry_type ,CR_contact_person ,CR_Address ,CR_email_id ,CR_phone_no ,CR_mobile_no ,CR_Website ,CR_username ,CR_password ,CR_payment_type,CR_date_time,CR_act_deact,CR_code_id,CR_Address from company_registration  where CR_code_id='"+id[j]+"'" );
		  rs=ps.executeQuery();
	      while(rs.next())
		  {
    ...
    <td width="19%" ><%=rs.getString("CR_name")%></td>
   
	<td width="17%"><%=rs.getString("CR_Address")%></td>
	
	<td width="17%"><%=rs.getString("CR_contact_person")%></td>
 
    <td width="12%" ><%=rs.getString("CR_phone_no")%></td>
	
	<td width="11%" ><%=rs.getString("CR_mobile_no")%></td>
	
	<td width="14%" ><%=rs.getString("CR_email_id")%></td>
...
<%}
  ...
  <tr>
    ...
    <td><a href="/admin/Companyactanddeactfree.jsp">Click here to go back to Company View List page</a></td>
  </tr>
</table>
</form>
<%}
