...
Connection conn=DriverManager.getConnection(conStr, dbUser, dbPwd);
  PreparedStatement ps1=conn.prepareStatement("select av_contact_company_name,av_contact_address,av_status_flag,av_contact_mobileno,av_contact_phoneno,av_contact_state,av_contact_city,av_id,av_contact_country from admin_vaccancy  where (av_status_flag IS NULL or av_status_flag='"+flag+"' or av_status_flag ='null') AND post_type='PAID_POST'");// and av_company_id='"+comptype+"' and  av_contact_company_name <> ''";
...
<body>
	...
	<% if(read1==1 && add1==1 && update1==1 && delete1==0)
   {
   %>
...
%>
	...
	<tr><td colspan="10"><div align="center" class="style1 style2"><strong>Job approval page for paid company posting</strong></div></td>
	 </tr>
	 <tr>
	   <td>Id</td>
	   <td width="16%">Company Name</td>
	   <td width="23%">Company Address</td>
	   <td width="5%">City</td>
	   <td width="6%">State</td>
	   <td width="9%">Country</td>
	   <td width="12%">Phone No</td>
	   <td width="12%">Mobile No</td>
	   <td width="7%">Status</td>
	   <td width="10%">Edit Job</td>
...
while(rs1.next())
   {
	//println("Inside");
	if ( i < (pageNo * 20)&&!rs1.getString("av_contact_company_name").equals(""))
	{
		//println("Inside1"+rs1.getString("av_contact_address")+"----------");
%>
	  
	  
   <tr>
   
    <td><%=rs1.getString("av_id")%></td>
	
    <td width="16%" ><%=rs1.getString("av_contact_company_name")%></td>
  
	<td width="23%" ><%=rs1.getString("av_contact_address")%></td>
   
    <td width="5%" ><%=rs1.getString("av_contact_city")%></td>
   
	<td width="6%"><%=rs1.getString("av_contact_state")%></td>
 
    <td width="9%" ><%=rs1.getString("av_contact_country")%></td>
	
	<td width="12%" ><%=rs1.getString("av_contact_phoneno")%></td>
	
	<td width="12%" ><%=rs1.getString("av_contact_mobileno")%></td>
	
	<td width="7%" ><%=rs1.getString("av_status_flag")%></td>
	<td width="10%"><a href="/admin/editjobpaid.jsp?id=<%=rs1.getString("av_id")%>">Edit Job</a></td>
...
%>
				...
				<a style="text-decoration:none" href="/admin/Newopeningspaid.jsp?pageNo=<%= i %>&amp;"> <%= i %></a>
<%
			}
			out.write(" "); 
		}		
